...
export const FAQ = () => {
  ...
  return <section className="py-32 bg-gradient-subtle bg-inherit">
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            {faqs.map((faq, index) => <Card key={index} className="shadow-card-brand border-2 border-primary/10 overflow-hidden">
                <CardContent className="p-0">
                  <button onClick={() => setOpenFaq(openFaq === index ? null : index)} className="w-full p-6 text-left hover:bg-muted/50 transition-colors flex items-center justify-between">
                    <h3 className="text-lg font-semibold">{faq.question}</h3>
                    {openFaq === index ? <ChevronDown className="text-primary" size={24} /> : <ChevronRight className="text-primary" size={24} />}
                  </button>
                  
                  {openFaq === index && <div className="px-6 pb-6 border-t border-border">
